...
class NhProjectsController < NeurohubApplicationController
  ...
  def files #:nodoc:
    cbrain_session['nh_files_per_page'] = Pagy::VARS[:items] if cbrain_session['nh_files_per_page'].blank?
    @nh_project   = find_nh_project(current_user, params[:id])
    @pagy, @files = pagy(@nh_project.userfiles, :items => cbrain_session['nh_files_per_page'].to_i)
  end
